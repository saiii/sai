  ...
  _receiver(0)
{
  ...
  if (!pgm_socket (&_sckt, family, SOCK_SEQPACKET, IPPROTO_UDP, &err)) 
  {
    // TODO log error err->message
    pgm_error_free (err);
    return;
  ...
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_UDP_ENCAP_UCAST_PORT, &port, sizeof(port));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_UDP_ENCAP_MCAST_PORT, &port, sizeof(port));

  const int no_router_assist = 0;
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_IP_ROUTER_ALERT, &no_router_assist, sizeof(no_router_assist));
  ...
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_MTU, &g_max_tpdu, sizeof(g_max_tpdu));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_TXW_SQNS, &g_sqns, sizeof(g_sqns));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_TXW_MAX_RTE, &g_max_rte, sizeof(g_max_rte));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_AMBIENT_SPM, &ambient_spm, sizeof(ambient_spm));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_HEARTBEAT_SPM, &heartbeat_spm, sizeof(heartbeat_spm));

  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_RXW_SQNS, &g_sqns, sizeof(g_sqns));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_PEER_EXPIRY, &peer_expiry, sizeof(peer_expiry));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_SPMR_EXPIRY, &spmr_expiry, sizeof(spmr_expiry));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_NAK_BO_IVL, &nak_bo_ivl, sizeof(nak_bo_ivl));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_NAK_RPT_IVL, &nak_rpt_ivl, sizeof(nak_rpt_ivl));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_NAK_RDATA_IVL, &nak_rdata_ivl, sizeof(nak_rdata_ivl));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_NAK_DATA_RETRIES, &nak_data_retries, sizeof(nak_data_retries));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_NAK_NCF_RETRIES, &nak_ncf_retries, sizeof(nak_ncf_retries));
  ...
  if (useFec) 
  {
    ...
    pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_USE_FEC, &fecinfo, sizeof(fecinfo));
  }
  ...
  {
    pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_JOIN_GROUP, &_addrInfo->ai_recv_addrs[i], sizeof(struct group_req));
  }
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_SEND_GROUP, &_addrInfo->ai_send_addrs[0], sizeof(struct group_req));
  ...
  const int blocking = 0,
            ...
            multicast_hops = 16,
            dscp = 0x2e << 2; 
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_MULTICAST_LOOP, &multicast_loop, sizeof(multicast_loop));
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_MULTICAST_HOPS, &multicast_hops, sizeof(multicast_hops));
  if (AF_INET6 != family)
  {
    pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_TOS, &dscp, sizeof(dscp));
  }
  pgm_setsockopt (_sckt, IPPROTO_PGM, PGM_NOBLOCK, &blocking, sizeof(blocking));
...
{
  const int status = pgm_send (_sckt, data, size, 0);
...
{ 
  _handler = h; 
  _receiver = new Receiver(_sckt, _handler); 
}

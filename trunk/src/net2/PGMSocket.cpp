...
public:
  Receiver(pgm_sock_t* sckt, RawDataHandler *);
  ~Receiver();
...
void 
NetworkOptions::copy(NetworkOptions* opt)
{
  _port = opt->_port;
  _interface.assign(opt->_interface);
  _send.assign(opt->_send);
  _withHttp = opt->_withHttp;
  _disablePGM = opt->_disablePGM;
  int size = opt->_receive.size();
  for (int i = 0; i < size; i += 1)
  {
    char * s = opt->_receive.at(i);
    _receive.push_back(strdup(s));
  }
}

bool 
NetworkOptions::equals(NetworkOptions* opt)
{
  if (_port != opt->_port) return false;
  if (_interface.compare(opt->_interface) != 0) return false;
  if (_send.compare(opt->_send) != 0) return false;
  if (_withHttp != opt->_withHttp) return false;
  
  if (_receive.size() != opt->_receive.size()) return false;

  int size = _receive.size();
  for(int i = 0; i < size; i += 1)
  {
    if (strcmp(_receive.at(0), opt->_receive.at(0)) != 0) return false;
  }
...
void 
PGMSocket::setEventHandler(RawDataHandler * h) 
{ 
  _handler = h; 
  _receiver = new Receiver(_scktRecv, _handler); 
}

Receiver::Receiver(pgm_sock_t* sckt, RawDataHandler* handler):
  _sckt(sckt),
  _handler(handler),
  _running(true)
{
      ...
      case PGM_IO_STATUS_NORMAL:
        {
          ...
          pgm_gsi_print_r (&(from.sa_addr.gsi), buf, sizeof(buf));
          Endpoint endpoint(buf, from.sa_port, PGM, 0);
          _handler->processDataEvent(&endpoint, buffer, (uint32_t)len);
        }
        break;
...
#endif
		  //printf("PGM_IO_STATUS_RATE_LIMITED\n");
        }
        break;
...
#endif
		//printf("PGM_IO_STATUS_WOULD_BLOCK\n");
        break;
      default:
        if (err)
        {
...
PGMSocketFactory * PGMSocketFactory::_instance = 0;

PGMSocketFactory::PGMSocketFactory()
{
}

PGMSocketFactory * 
PGMSocketFactory::GetInstance()
{
  if (!_instance)
  {
    _instance = new PGMSocketFactory();
  }
  return _instance;
}
...
PGMSocket * 
PGMSocketFactory::create(NetworkOptions * options)
{
  PGMSocketInfoListIterator iter;
  for (iter  = _list.begin();
       iter != _list.end();
       iter ++)
  {
    PGMSocketInfo * info = *iter;
    if (options->equals(info->options))
    {
      return info->sckt;
    }
  }

  PGMSocketInfo * ret = new PGMSocketInfo();
  ret->sckt = new PGMSocket(options);
  ret->options = new NetworkOptions();
  ret->options->copy(options);
  _list.push_back(ret);
  return ret->sckt;
}
